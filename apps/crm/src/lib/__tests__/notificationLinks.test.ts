...
describe('CRM resolveNotificationLink', () => {
  beforeEach(() => {
  ...
  it.each(CRM_ROUTE_PREFIXES)('keeps the CRM /%s route in the current app', (prefix) => {
  ...
  it.each(HRM_ROUTE_PREFIXES)('resolves the HRM /%s route across apps', (prefix) => {
    const link = `/${prefix}/example`;

    expect(resolveNotificationLink(link)).toEqual({
      href: `https://hrm.example.test${link}`,
      isCrossApp: true,
    });
  });
  ...
  it('preserves query strings and hashes on a current-app route', () => {
    const link = '/portal-clinical/notes?status=unsigned#note-42';
  ...
  it('falls back to the HRM localhost URL when its public URL is absent', () => {
    delete process.env.NEXT_PUBLIC_HRM_URL;

    expect(resolveNotificationLink('/rbt/schedule')).toEqual({
      href: 'http://localhost:3001/rbt/schedule',
      isCrossApp: true,
    });
  });
