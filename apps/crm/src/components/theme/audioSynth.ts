...
export function playClickSound(mode: ClickSoundMode, volume: number = 0.5) {
  ...
  if (mode === 'subtle-click') {
    // Mechanical switch click: high-frequency pop + snappy noise transient
    ...
    // Holographic laser blip: fast upward frequency sweep with bright resonance
    const osc = ctx.createOscillator();
    osc.type = 'sine';
    ...
    // Liquid water bubble pop: downward pitch sweep with resonant filter
    const osc = ctx.createOscillator();
    osc.type = 'sine';
    ...
    // Cosmic chime: harmonic chord with sub-bass pulse
