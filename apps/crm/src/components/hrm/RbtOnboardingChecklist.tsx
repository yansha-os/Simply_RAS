...
export default function RbtOnboardingChecklist() {
  ...
  useEffect(() => {
    const checkInterviewData = () => {
      ...
      if (interviewDone && payloadStr) {
        try {
          const payload = JSON.parse(payloadStr);
          setCandidates(prev => prev.map(c => {
            if (c.id === '1' || c.name.includes('David')) {
              return {
                ...c,
    ...
    return () => window.removeEventListener('rbt_interview_changed', checkInterviewData);
  }, []);
  ...
  const handlePassInterview = (candidate: OnboardingCandidate) => {
    ...
    // Save clearance to localStorage so RBT candidate screen immediately unlocks!
