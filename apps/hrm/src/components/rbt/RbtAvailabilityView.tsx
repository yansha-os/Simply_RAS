...
interface RbtAvailabilityViewProps {
  mode?: 'availability' | 'job-board';
}

export function RbtAvailabilityView({ mode = 'availability' }: RbtAvailabilityViewProps) {
  ...
  useEffect(() => {
    ...
    const checkStatus = () => {
      ...
      if (availabilitySet) {
        ...
        const emptyGrid = Array.from({ length: 7 }, () => Array(13).fill(false));
        setGrid(emptyGrid);
      }
    };
  ...
  const all4Done = reqStatus.tasksDone && reqStatus.interviewBooked && reqStatus.availabilitySet && reqStatus.simCompleted;
  const isJobBoardUnlocked = mode === 'availability' ? true : (reqStatus.isCleared || all4Done);
  ...
  if (mode === 'availability' && isAvailabilitySaved && !showGridEditor) {
    return (
      <div className="max-w-2xl mx-auto py-12 px-6 text-center select-none space-y-6 animate-fade-in">
        <div className="bg-white border-4 border-emerald-300 rounded-3xl p-8 sm:p-10 shadow-2xl space-y-6">
          ...
          <div className="space-y-2">
            ...
            <h2 className="text-2xl font-black font-heading text-slate-900 tracking-tight">
              Weekly Work Availability Grid Configured
            </h2>
            <p className="text-xs text-slate-600 font-semibold max-w-lg mx-auto leading-relaxed">
              Great job! Your preferred work availability hours and travel preferences have been configured and saved. This requirement is complete. Please finish any remaining onboarding tasks on your <strong>My Tasks</strong> page.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3 pt-2">
            <Link
              href="/rbt"
              className="inline-flex items-center justify-center gap-2 bg-[#F97316] hover:bg-orange-600 text-white font-black text-xs px-6 py-3.5 rounded-2xl shadow-xl transition-all cursor-pointer"
            >
              <ClipboardList className="w-4.5 h-4.5" />
            ...
            <button
              ...
              className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs px-5 py-3.5 rounded-2xl cursor-pointer transition-all flex items-center justify-center gap-2"
            >
              <Edit3 className="w-4 h-4" />
              <span>Edit Availability Grid</span>
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      {/* VIEW 1: DRAG-TO-SELECT AVAILABILITY GRID (ALWAYS FOR MODE AVAILABILITY) */}
      {(mode === 'availability' || !isAvailabilitySaved || showGridEditor) && (
      ...
      {isAvailabilitySaved && !showGridEditor && (
        <div className="space-y-6 animate-fade-in">
          {!all4Done && (
            <div className="p-6 bg-amber-50 border-2 border-amber-300 rounded-3xl space-y-3 text-amber-950 shadow-md">
              <div className="flex items-center gap-2">
                <Lock className="w-5 h-5 text-[#F97316]" />
                <h4 className="text-sm font-black font-heading text-slate-900">Job Board Locked Until All Onboarding Tasks Are Complete</h4>
              </div>
              <p className="text-xs text-slate-700 font-semibold leading-relaxed">
                Your Weekly Availability Grid has been successfully recorded! However, applying to client cases requires completing <strong>all 4 core onboarding tasks</strong> (My Tasks, Data Simulator, Availability, and HR Interview).
              </p>
            </div>
          )}
                ...
                >
                  ...
                  <div className="bg-orange-50/80 border border-orange-200 p-3 rounded-2xl text-xs font-bold text-slate-800 flex items-center justify-between gap-3">
                    ...
                    {c.applied ? (
                      ...
                      <button
                        onClick={() => all4Done ? handleOpenApplyModal(c) : toast.error('Complete all 4 onboarding requirements to apply for cases!')}
                        disabled={!all4Done}
                        className={`text-white font-black text-xs px-5 py-2.5 rounded-xl shadow-md flex items-center gap-1.5 transition-all shrink-0 ${
                          all4Done ? 'bg-[#F97316] hover:bg-orange-600 cursor-pointer' : 'bg-slate-400 cursor-not-allowed opacity-75'
                        }`}
                      >
                        <Briefcase className="w-3.5 h-3.5" />
                        <span>{all4Done ? 'Apply & Request Family Meet & Greet →' : '🔒 Onboarding Pending'}</span>
                      </button>
                    )}
                  </div>
                </div>
