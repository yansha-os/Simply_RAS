...
- **Health endpoints** — `GET /api/health` on **both** apps (CRM `:3000`, HRM `:3001`). Unauthenticated by design (uptime checkers must reach them); response is deliberately minimal: `{ ok, app, time, db }`. Each does a fast `SELECT 1` DB probe with a 3s timeout. **200** = app + DB healthy. **503** = the app process is up but the DB probe failed — think Supabase outage, transaction-pooler connection exhaustion, or a bad `DATABASE_URL`; check Supabase logs (below) before restarting anything. No response at all = the Next process itself is down.
...
- **Prod smoke after any deploy:** both `/api/health` return 200 with `"db":"ok"`; both `/login` pages load; one CRM client profile renders; HRM RBT portal renders; no `[SECURITY]` or Prisma errors in logs.
