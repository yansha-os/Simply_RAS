...
| 10 | [`2026-08-16-232000Z-evv-aggregator-sync-fields.sql`](2026-08-16-232000Z-evv-aggregator-sync-fields.sql) | Add State EVV aggregator sync fields (vendor, status, responseId, submittedAt) to `EVVLog` table — **HOLD** until a real aggregator is chosen, enrolled, and certified; the synthetic adapter code was retired |
...
| 13 | [`2026-08-20-012800Z-dual-run-cohort-fields.sql`](2026-08-20-012800Z-dual-run-cohort-fields.sql) | Legacy `Client.dualRunCohort` + `Client.dualRunMode` fields — **HOLD** for new targets because the `/portal-billing/audit` worksheet was retired; existing columns may remain until a separately reviewed cleanup |
...
| 21 | [`2026-09-07-192400Z-public-schema-least-privilege.sql`](2026-09-07-192400Z-public-schema-least-privilege.sql) | Revoke public-table/sequence/function access from Supabase REST roles and harden `postgres` default privileges — **verified applied to `Simple_RAS_CRM_DEV` on 2026-09-07**; public application data remains server-only through Prisma |
| 22 | [`2026-09-07-200123Z-null-placeholder-medicaid-ids.sql`](2026-09-07-200123Z-null-placeholder-medicaid-ids.sql) | Guardedly replace three recognized DEV placeholder Medicaid identifiers with `NULL`; never merges/deletes clients — **verified applied to `Simple_RAS_CRM_DEV` on 2026-09-07** |
| 23 | [`2026-09-08-010551Z-magic-link-expiry-fail-closed.sql`](2026-09-08-010551Z-magic-link-expiry-fail-closed.sql) | Backfill missing parent/applicant link expiries from original issuance time and prevent future live tokens without expiry — **verified applied to `Simple_RAS_CRM_DEV` on 2026-09-07**; no dependency beyond order 6 columns |
| 24 | [`2026-09-12-060419Z-multistate-rule-foundation.sql`](2026-09-12-060419Z-multistate-rule-foundation.sql) | Add multi-state ownership, payer/enrollment, immutable effective rule versions, and durable session resolution snapshots — **verified applied to `Simple_RAS_CRM_DEV` on 2026-09-12**; additive and depends only on the existing `User`, `Client`, and `Session` tables |
...
| [`2026-08-20-012800Z-dual-run-cohort-fields.sql`](2026-08-20-012800Z-dual-run-cohort-fields.sql) | Legacy sandbox/dual-run cohort fields — **HOLD** on new targets; the owning worksheet was retired |
...
| [`2026-09-07-192400Z-public-schema-least-privilege.sql`](2026-09-07-192400Z-public-schema-least-privilege.sql) | Defense-in-depth least privilege for public-schema tables, sequences, functions, and future `postgres` objects |
| [`2026-09-07-200123Z-null-placeholder-medicaid-ids.sql`](2026-09-07-200123Z-null-placeholder-medicaid-ids.sql) | Guarded DEV cleanup of recognized placeholder Medicaid identifiers without exposing their values or changing client identity |
| [`2026-09-08-010551Z-magic-link-expiry-fail-closed.sql`](2026-09-08-010551Z-magic-link-expiry-fail-closed.sql) | Fail-closed expiry backfill and database constraints for parent/applicant magic links |
| [`2026-09-12-060419Z-multistate-rule-foundation.sql`](2026-09-12-060419Z-multistate-rule-foundation.sql) | Additive multi-state organization/location/jurisdiction/payer/enrollment authority plus immutable effective rule versions and content-addressed session decision snapshots — **verified applied to `Simple_RAS_CRM_DEV` on 2026-09-12** |
